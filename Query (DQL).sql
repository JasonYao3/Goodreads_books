...
SELECT 
    ROUND((SUM(CASE
                WHEN avg_rating >= 0 AND avg_rating < 1 THEN 1
                ELSE 0
            END) / COUNT(book_id)) * 100,
            2) 'Percentage Ratings Between 0 and 1',
    ROUND((SUM(CASE
                WHEN avg_rating >= 1 AND avg_rating < 2 THEN 1
                ELSE 0
            END) / COUNT(book_id)) * 100,
            2) 'Percentage Ratings Between 1 and 2',
    ROUND((SUM(CASE
                WHEN avg_rating >= 2 AND avg_rating < 3 THEN 1
                ELSE 0
            END) / COUNT(book_id)) * 100,
            2) 'Percentage Ratings Between 2 and 3',
    ROUND((SUM(CASE
                WHEN avg_rating >= 3 AND avg_rating < 4 THEN 1
                ELSE 0
            END) / COUNT(book_id)) * 100,
            2) 'Percentage Ratings Between 3 and 4',
    ROUND((SUM(CASE
                WHEN avg_rating >= 4 THEN 1
                ELSE 0
            END) / COUNT(book_id)) * 100,
            2) 'Percentage Ratings Greater than 4'
FROM
    book;

SELECT 
    publisher, COUNT(title) AS 'books published'
FROM
    book
        INNER JOIN
    publish USING (publisher_id)
GROUP BY publisher
ORDER BY COUNT(title) DESC
LIMIT 10;
